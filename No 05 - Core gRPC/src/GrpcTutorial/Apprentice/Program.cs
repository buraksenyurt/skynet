...
namespace Apprentice
{
    class Program
    {
        ...
        {
            // Ubuntu tarafındaki "Error starting gRPC call: The SSL connection could not be established" hatası nedeniyle eklendi
            var httpClientHandler = new HttpClientHandler();
            httpClientHandler.ServerCertificateCustomValidationCallback = HttpClientHandler.DangerousAcceptAnyServerCertificateValidator;
            var httpClient = new HttpClient(httpClientHandler);
            using var channel = GrpcChannel.ForAddress("https://localhost:5555",new GrpcChannelOptions { HttpClient = httpClient });
            ...
            var recipe = await client.GetRecipeAsync(new GetRecipeRequest { RecipeId = 1 });
